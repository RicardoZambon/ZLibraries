...
`@Input() type: string` with no setter, transform, or attribute read, so the _class property_ is identical
...
the routed form's host. That is why Panthor's _form_ components need no SCSS while all its list components
...
_statically-set_ attribute. Angular applies host bindings after creation and `[attr.x]="null"` calls
...
generated by each _consumer's_ Tailwind — a consumer that doesn't scan `node_modules/@zambon-dev/**`
...
`host: { class: 'framework-view-list' }` is a host _class_, not a selector, so the prefix rules never apply
to it.
