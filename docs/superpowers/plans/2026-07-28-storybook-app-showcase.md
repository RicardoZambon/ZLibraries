...
5. **`ButtonSaveComponent` reads `model.id` from the saved model** to build the post-save URL. `FormService.getModelFromForm()` returns `form.getRawValue()`, which has **no `id`** (no `id` control). Therefore mock `saveModel` **must** return an object that includes `id`.
   ...
   - **`saveModel` should upsert into the seed array**, the way a real backend persists, so the list view reflects edits and newly created records. Use the shared `upsertById(rows, row)` helper. Build the saved row **field-by-field** rather than spreading the incoming model — the model carries no `id`, and spreading would also drag detail-only fields into the persisted list row.
   - **Type the save parameter as `Omit<IXxxDisplay, 'id'>`**, not `IXxxDisplay`. The value is the form's raw value and genuinely has no `id`; the `Omit` turns any attempt to read `model.id` into a compile error. The base declares `saveModel(model: any)`, so narrowing is allowed.
   - **Detail-only fields need a sibling store.** A field present on the detail model but not the list row (e.g. `mustChangePassword`) will not round-trip if only the list row is persisted — it silently reverts on reopen. Keep it in a `Set<number>`/`Map` beside the seed array and read it back in the `findXxx` lookup.
